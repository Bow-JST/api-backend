...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Override
    public boolean updateUserInfo(UserUpdateSecret secret, HttpServletRequest request) {
        User loginUser = getLoginUser(request);
        String accessKey = secret.getAccessKey();
        String secretKey = secret.getSecretKey();
        // 一个月只能更新一次 ak sk
        LocalDateTime last = loginUser.getUpdateTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
        LocalDateTime current = LocalDateTime.now();
        Duration between = Duration.between(current, last);
        if(between.toDays() < 30){
            return false;
        }
        // 查询 accessKey 是否已经存在了
        ...
        // ak sk只能包含字母或数字
        if(!accessKey.matches("[a-zA-Z0-9]+") || !secretKey.matches("[a-zA-Z0-9]+")){
            return false;
        }
        // 执行更新操作
