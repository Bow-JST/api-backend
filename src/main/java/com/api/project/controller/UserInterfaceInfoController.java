...
public class UserInterfaceInfoController {
    ...
    /**
     * 创建
     * @param userInterfaceInfoAddRequest 添加接口调用请求体
    ...
    /**
     * 更新
     * @param userInterfaceInfoUpdateRequest 更新接口调用请求体
     * @param request request 请求
     * @return 结果
     */
    @PostMapping("/update")
    ...
    /**
     * 获取列表（仅管理员可使用）
     * @param userInterfaceInfoQueryRequest 查询接口调用请求体
     * @return 结果
     */
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    @GetMapping("/list")
    ...
    public BaseResponse<Boolean> updateUserInfo(@RequestBody UserInterfaceInfoRecord record, HttpServletRequest request) {
        boolean result = userInterfaceInfoService.updateUserInterfaceInfo(record, request);
        return result ? ResultUtils.success(result) : ResultUtils.error(ErrorCode.OPERATION_ERROR);
    }
}
